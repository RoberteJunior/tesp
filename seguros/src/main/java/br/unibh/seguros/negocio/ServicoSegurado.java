...
public class ServicoSegurado implements DAO<Segurado, Long> {
	...
	@SuppressWarnings("unchecked")
	public List<Segurado> findByNameComFuncionarios(String name) throws Exception {
		log.info("Encontrando o " + name);
		return em.createNamedQuery("Segurado.findByNameComFuncionarios").setParameter("nome", name + "%").getResultList();
	}
	
	@SuppressWarnings("unchecked")
	public List<Segurado> findByIDComSeguradoSuperior(Long id) throws Exception {
		log.info("Encontrando o " + id);
		return em.createNamedQuery("Segurado.findByIDComSeguradoSuperior").setParameter("id", id).getResultList();
	}
}
